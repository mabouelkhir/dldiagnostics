...
class PatientHomePageState extends State<PatientHomePage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance!.addPostFrameCallback((_) async {
      try {
        var userData = await getActualUser();
        var patient = PatientModel.fromJson(userData['patient']);
        print("The patient id is: ${patient.id}");

        setState(() {
          DrawerPatientScreenState.idPatient = patient.id;
        });

        print("Updated idPatient: ${DrawerPatientScreenState.idPatient}");

        // You don't need to call `storeUser` inside this callback unless it's asynchronous too.
        setState(() {
          LoginPageState.username;
          LoginPageState.firstName=patient.nom;
          LoginPageState.lastName=patient.prenom;

          PatientHomePageState.idPatient=patient.id;
