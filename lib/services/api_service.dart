...
class ApiService {
  ...
  Future<Response> getRendezVousByPatient() async {
    var rendezVousUrl = Uri.parse('${Constants.BASE_URL}/rendez-vous/patient');
  ...
  Future<Response> getRendezVousByMedecin() async {
    var rendezVousUrl = Uri.parse('${Constants.BASE_URL}/rendez-vous/medecin');
  ...
  Future<Response> getRendezVousBySecretaire() async {
    var rendezVousUrl = Uri.parse('${Constants.BASE_URL}/rendez-vous/secretaire');
  ...
  Future<Response> getDiagnosticResult(String base64Image, MaladieModel maladie, PatientModel patient) async {
  ...
  Future<Response> ValidateAppointement(AppointmentModel model) async {
    var AppUrl = Uri.parse('${Constants.BASE_URL}/rendez-vous/${model!.id}');
  ...
  Future<Response> getVisitesByMedecin() async {
    var visitesUrl = Uri.parse('${Constants.BASE_URL}/visite/medecin');
  ...
  Future<List<MaladieModel>> getMaladies() async {
    try {
      var maladiesUrl = Uri.parse('${Constants.BASE_URL}/maladies');
      var token = await LoginPageState.storage.read(key: "token");
      ...
      if (res.statusCode == 200) {
        Iterable maladiesJson = jsonDecode(res.body);
        List<MaladieModel> maladies =
        maladiesJson.map((e) => MaladieModel.fromJson(e)).toList();
        print(maladies);
        return maladies;
      } else {
        throw Exception('Failed to load maladies. Status code: ${res.statusCode}');
      }
    } catch (e) {
      print('Error fetching maladies: $e');
      throw Exception('Failed to load maladies. Error: $e');
    }
  }
